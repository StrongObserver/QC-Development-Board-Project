...
| HardwareBuffer Stage D reachability | CameraX `ImageProxy.image.hardwareBuffer` -> native `AHardwareBuffer_lockPlanes` | status pass; format `35`, desc `1280x960`, 3 planes, plane0 rowStride `1280`, pixelStride `1` | Android buffer API is reachable from the current ImageAnalysis frame | QNN memory registration | `C:\Users\Admin\Videos\RB5 gen2\RB5_SR_Benchmark_v1\results\20260723_hardware_buffer_probe_stage_d` |
| Native YUV loop breakdown | Single-frame direct-YUV ROI/RGB internal timing | address `6-8us`, output pin `1-2us`, release `2us`; loop dominates around `1.3-2.0ms` | JNI overhead is small; pixel loop is the next optimization target | Sustained live timing by itself | `20260723_native_yuv_breakdown_internal`; `20260723_native_yuv_breakdown_integer_math_v2` |
| Integer YUV math live check | Current app default after integer YUV formula | direct-vs-array MAD `0.0`; direct path `2ms`; live e2e `8/9/9ms`, YUV fill `4.4/5.4/5.7ms` | Small safe math-path optimization evidence | Major performance gain | `20260723_direct_yuv_integer_math_probe`; `20260723_integer_yuv_live_roi_120f` |
...
| QNN Delegate profile diagnostic | rechecked on current APK and internal-AI-confirmed | fixed-sample raw delegate profile is 904 bytes; 10/10 known event strings recognized; `qnn-profile-viewer` still rejects the buffer, so this is diagnostic-only; official profiling should use qnn-net-run/native QNN route |
