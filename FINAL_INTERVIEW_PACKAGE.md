...
63/65ms 推到最新 direct-YUV native staging 默认路径 8/9/9ms p50/p95/p99，并用固定 benchmark、
...
和 UINT8 tensor bulk-copy；进一步复用 native RGB staging buffer 后，20 分钟
live run 达到 direct-YUV 默认路径 8/9/9ms p50/p95/p99。
...
| Current default native-staging live e2e | `8/9/9ms` p50/p95/p99 |
| Current board-level live power | about `4.96W` mean, battery-node estimate |
...
output reuse, UINT8 bulk-copy, and native RGB staging then pushed the current direct-YUV default to 8/9/9ms p50/p95/p99 over 20 minutes.
```
