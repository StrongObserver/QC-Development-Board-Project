...
- 将默认 live ROI 路线切换为 `QNN + QuickSRNetSmall W8A8`，保留 Real-ESRGAN 作为 QNN/HTP 里程碑、感知增强/文字边缘对照与可选 post-capture 路径；默认 live ROI smoke 95 帧验证 `QNN inference p50/p95=1.0/1.0ms`、`app e2e p50/p95=19.0/26.3ms`。
...
- 完成 Kotlin-only YUV ROI correctness probe：与当前 Bitmap ROI 路径 `MAD=0.34`，但 `yuvRoiMs=16ms` 慢于当前 `toBitmap+crop`，因此未替换默认路径；下一步性能线应转向 native C++ 或 tensor-ready buffer。
...
- Switched the default live-ROI route to `QNN + QuickSRNetSmall W8A8`, while keeping Real-ESRGAN as the QNN/HTP milestone, perceptual/text-edge comparison path, and optional post-capture enhancement path. Default live-ROI smoke validation parsed 95 frames with `QNN inference p50/p95=1.0/1.0ms` and `app e2e p50/p95=19.0/26.3ms`.
...
- Completed a Kotlin-only YUV ROI correctness probe: it matched the current Bitmap ROI path (`MAD=0.34`) but was slower (`yuvRoiMs=16ms`), so it was not promoted to the default live path. The next performance lane should move to native C++ or tensor-ready buffers.
