...
- 将默认 live ROI 路线切换为 `QNN + QuickSRNetSmall W8A8`，保留 Real-ESRGAN 作为 QNN/HTP 里程碑、感知增强/文字边缘对照与可选 post-capture 路径；输出复用后默认 live ROI 验证 `QNN inference p50/p95=1.0/1.0ms`、`app e2e p50/p95=19.0/24.7ms`。
- 进一步优化 app 后处理路径，复用 TFLite input/output buffer、像素数组和 UINT8 输出 byte buffer，减少每帧 sample copy 与 direct buffer 逐通道读取；最新默认 live ROI smoke 达到 `postprocess p50/p95=1/1ms`、`app e2e p50/p95=15/19ms`，60 秒 sustained smoke 首尾 e2e 为 `15/20ms -> 16/21ms`。
...
- Switched the default live-ROI route to `QNN + QuickSRNetSmall W8A8`, while keeping Real-ESRGAN as the QNN/HTP milestone, perceptual/text-edge comparison path, and optional post-capture enhancement path. After output reuse, default live-ROI validation reached `QNN inference p50/p95=1.0/1.0ms` and `app e2e p50/p95=19.0/24.7ms`.
- Optimized app postprocess overhead by reusing TFLite input/output buffers, pixel arrays, and a UINT8 output byte buffer, reducing live evidence copying and per-channel direct-buffer reads; latest default live-ROI smoke reached `postprocess p50/p95=1/1ms` and `app e2e p50/p95=15/19ms`, with a 60-second sustained smoke drifting from `15/20ms` to `16/21ms`.
...
- Evaluated Kotlin/native YUV ROI and tensor-ready input paths. Kotlin YUV ROI was correct but slower; native ROI was faster in a single-frame probe; tensor-ready input showed single-frame promise but did not beat the default Bitmap path in repeated live p50. Kept the default route and used output Bitmap reuse to reduce tail latency. A 120-second default live run parsed 3551 frames with e2e first/last p50/p95 of `20.0/25.0ms -> 21.0/26.0ms`.
