...
| video | video every-N-frame enhancement | done | `20260720_demo_mode_wide_fov_20s` records the Demo Mode live ROI UI with `adb screenrecord`; it uses a wider 960->128 crop and preserves button access through default UI / overlay toggle / intent control | Treat as demo evidence only; full CameraX VideoCapture/Recorder waits for explicit product need |
