...
| zero-copy | true CameraX buffer -> QNN input binding | queued | Invoke-level shared custom allocation, direct-plane probes, and phase3 camera direct-YUV -> normal/custom QNN tensor compare are complete with matching output checksum. This proves useful near-zero-copy/custom-allocation evidence, but still not CameraX YUV buffer registration. | Pursue true buffer registration only as larger Stage D project beyond the current 8/9/9ms native-staging baseline |
...
| temporal | every-N cadence and demo video boundary | done | `sr_every_n=3` now works on the optimized tensor default path: current recheck enhanced 287 frames, skipped 574, effective enhanced FPS p50/p95 9.8/9.8; Demo Mode screenrecord boundary exists | Do not claim full VideoCapture/Recorder or lower per-frame latency |
